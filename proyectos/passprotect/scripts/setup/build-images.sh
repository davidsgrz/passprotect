...
# Login en Docker Hub
echo "[0/5] Login en Docker Hub..."
docker login -u "$REGISTRY" || { echo "ERROR: Login fallido"; exit 1; }

# Capa 1: ubbase
echo "[1/5] Construyendo ubbase..."
docker build -t "${REGISTRY}/ubbase:latest" -f docker/kube/base/Dockerfile .
docker push "${REGISTRY}/ubbase:latest"

# Capa 2: ubseguridad
echo "[2/5] Construyendo ubseguridad..."
docker build -t "${REGISTRY}/ubseguridad:latest" -f docker/kube/seguridad/Dockerfile .
docker push "${REGISTRY}/ubseguridad:latest"

# Capa 3a: vaultwarden-corp
echo "[3/5] Construyendo vaultwarden-corp..."
docker build -t "${REGISTRY}/vaultwarden-corp:1.0.0" -f docker/kube/vaultwarden/Dockerfile .
docker push "${REGISTRY}/vaultwarden-corp:1.0.0"

# Capa 3b: keycloak-corp
echo "[4/5] Construyendo keycloak-corp..."
docker build -t "${REGISTRY}/keycloak-corp:1.0.0" -f docker/kube/keycloak/Dockerfile .
docker push "${REGISTRY}/keycloak-corp:1.0.0"

# Capa 3c: mariadb-corp
echo "[5/5] Construyendo mariadb-corp..."
docker build -t "${REGISTRY}/mariadb-corp:1.0.0" -f docker/kube/mariadb/Dockerfile .
docker push "${REGISTRY}/mariadb-corp:1.0.0"

# Capa 3d: nginx-proxy-corp
echo "[6/5] Construyendo nginx-proxy-corp..."
docker build -t "${REGISTRY}/nginx-proxy-corp:1.0.0" -f docker/kube/nginx-proxy/Dockerfile .
